...
// and mints unique $POG tokens based on their market cap rank.
(async () => {
  try {
    ...
    for (const crypto of cryptoData) {
      ...
      if (pogAmount > 0) {
        for (let i = 1; i <= pogAmount; i++) {
          const batchId = Date.now();
          const mintDate = new Date().toISOString();

          console.log(`Minting $POG #${i}/${pogAmount} for ${crypto.name}`);

          // Execute the Solana CLI command to mint tokens with metadata
          exec(
            `solana-token mint $MILKCAP_MINT_ADDRESS $RECIPIENT_ADDRESS 1 --metadata "{\"Batch ID\": \"${batchId}\", \"Type\": \"${crypto.symbol.toUpperCase()}\", \"Subdata\": \"Price: $${crypto.current_price}\", \"ID\": \"${i}/${pogAmount}\", \"Mint Date\": \"${mintDate}\"}"`,
            (error, stdout, stderr) => {
              if (error) {
                // Log an error if token minting fails
                console.error(`Error minting token ${i}: ${stderr}`);
              } else {
                // Log success if tokens were minted successfully
                console.log(`Successfully minted $POG #${i}/${pogAmount} for ${crypto.name}`);
              }
            }
          );
        }
